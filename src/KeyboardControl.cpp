...
bool KeyboardControl::Open() {
  ...
  if(deviceNode_.empty()) {
    LOG_E("Udev was unable to find a keyboard device");
    return false;
  }
...
bool KeyboardControl::Close(){
 ...
 arm_event KeyboardControl::Read(){
   int rc;
...
int KeyboardControl::GetBattery() {
  return 100;
}

void KeyboardControl::SetNotification(arm_notification notif){
  //coming soon
}
...
//  Private methods
//------------------------------------------------------------------------------
arm_event KeyboardControl::initEvent(arm_event movement){
...
bool KeyboardControl::FindDevnode(udev_device* dev) {
...
void JoystickPS3::SetMovement(struct input_event* ev)/*, arm_event movement)*/ {
  ...
  /*if(libevdev_event_is_code(ev, EV_ABS, ABS_Z) == 1) {
      if(ev->value > 140) {
        movement_.AbsStatus = movement_.AbsStatus &~ mask_abs_z_left;
        movement_.AbsStatus = movement_.AbsStatus |  mask_abs_z_right;
      }
      else if(120 < ev->value && 140 > ev->value){
        movement_.AbsStatus = movement_.AbsStatus &~ mask_abs_z_left;
        movement_.AbsStatus = movement_.AbsStatus &~ mask_abs_z_right;
      ...
      movement_.valueAbs[GRIPPER] = ev->value;
#ifdef
      LOG_D("event ABS_y (value=%d) => %s", ev->value, binaire(16, movement_.AbsStatus));
#endif
  }
  ...
  if(libevdev_event_is_code(ev, EV_ABS, ABS_RX) == 1) {
      if(ev->value > 140) {
        movement_.AbsStatus = movement_.AbsStatus &~ mask_abs_rz_left;
        movement_.AbsStatus = movement_.AbsStatus |  mask_abs_rz_right;
      }
      else if(120 < ev->value && 140 > ev->value){
        movement_.AbsStatus = movement_.AbsStatus &~ mask_abs_rz_left;
        movement_.AbsStatus = movement_.AbsStatus &~ mask_abs_rz_right;
      ...
      movement_.valueAbs[WRIST_ROT] = ev->value;
#ifdef
      LOG_D("event ABS_y (value=%d) => %s", ev->value, binaire(16, movement_.AbsStatus));
#endif
}*/
