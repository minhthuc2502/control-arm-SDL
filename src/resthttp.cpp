...
bool RestHttp::ResponseRestRequest(const string &url,
  ...
  if (url == "/initposition") {
    ...
    ptit->second.push_back(
        make_pair("error", pt::ptree(string("can't recognize url"))));
...
bool RestHttp::_open_arm_rest(const string &url,
                              const map<string, string> &argval,
                              string &response, ServerController &server) {
  int ret;
  ptree openroot;
...
bool RestHttp::_init_position_arm_rest(const string &url,
                                       const map<string, string> &argval,
                                       string &response,
...
bool RestHttp::_control_shoulder_rest(const string &url,
                                      const map<string, string> &argval,
                                      string &response,
                                      ServerController &server) {
  ...
  if (it == argval.end()) {
    ptit->second.push_back(make_pair("status", pt::ptree(string("OK"))));
    ptit->second.push_back(make_pair(
        "error", pt::ptree(string("can not get direction parameters"))));
    _generate_json_output(&shoulderroot, response);
    return false;
  } else if (strcmp(it->second.c_str(), "0") != 0 &&
             strcmp(it->second.c_str(), "1") != 0) {
    ptit->second.push_back(make_pair("status", pt::ptree(string("Failed"))));
    ptit->second.push_back(
        make_pair("error", pt::ptree(string("value of direction is wrong"))));
    _generate_json_output(&shoulderroot, response);
    return false;
  }
  if (!(ret = server.MoveShoulder(atoi(it->second.c_str())))) {
    ...
    ptit->second.push_back(make_pair(
        "positionInit",
        pt::ptree(to_string(server.GetJoint(SHOULDER).initPosition))));
    ptit->second.push_back(
        make_pair("hightLimit",
                  pt::ptree(to_string(server.GetJoint(SHOULDER).limitHight))));
    ptit->second.push_back(make_pair(
        "lowLimit", pt::ptree(to_string(server.GetJoint(SHOULDER).limitLow))));
    ptit->second.push_back(make_pair("status", pt::ptree(string("OK"))));
  }
  _generate_json_output(&shoulderroot, response);
  return true;
...
bool RestHttp::_control_elbow_rest(const string &url,
                                   const map<string, string> &argval,
                                   string &response, ServerController &server) {
  map<string, string>::const_iterator it;
  int ret;
  ...
  if (!(ret = server.MoveElbow(atoi(it->second.c_str())))) {
    ptit->second.push_back(
        make_pair("positionCurrent",
                  pt::ptree(to_string(server.GetJoint(ELBOW).actualPosition))));
    ptit->second.push_back(make_pair("status", pt::ptree(string("Failed"))));
    _generate_json_output(&elbowroot, response);
    return false;
  } else {
    ptit->second.push_back(
        make_pair("positionCurrent",
                  pt::ptree(to_string(server.GetJoint(ELBOW).actualPosition))));
    ptit->second.push_back(
        make_pair("positionInit",
                  pt::ptree(to_string(server.GetJoint(ELBOW).initPosition))));
    ptit->second.push_back(make_pair(
        "hightLimit", pt::ptree(to_string(server.GetJoint(ELBOW).limitHight))));
    ptit->second.push_back(make_pair(
        "lowLimit", pt::ptree(to_string(server.GetJoint(ELBOW).limitLow))));
    ptit->second.push_back(make_pair("status", pt::ptree(string("OK"))));
  }
  _generate_json_output(&elbowroot, response);
  return true;
...
bool RestHttp::_control_base_rest(const string &url,
                                  const map<string, string> &argval,
                                  string &response, ServerController &server) {
  map<string, string>::const_iterator it;
  int ret;
  ...
  if (!(ret = server.RotateBase(atoi(it->second.c_str())))) {
    ptit->second.push_back(
        make_pair("positionCurrent",
                  pt::ptree(to_string(server.GetJoint(BASE).actualPosition))));
    ptit->second.push_back(make_pair("status", pt::ptree(string("Failed"))));
    _generate_json_output(&baseroot, response);
    return false;
  } else {
    ...
    ptit->second.push_back(
        make_pair("positionInit",
                  pt::ptree(to_string(server.GetJoint(BASE).initPosition))));
    ptit->second.push_back(make_pair(
        "hightLimit", pt::ptree(to_string(server.GetJoint(BASE).limitHight))));
    ptit->second.push_back(make_pair(
        "lowLimit", pt::ptree(to_string(server.GetJoint(BASE).limitLow))));
    ptit->second.push_back(make_pair("status", pt::ptree(string("OK"))));
  }
  _generate_json_output(&baseroot, response);
  return true;
...
bool RestHttp::_control_gripper_rest(const string &url,
                                     const map<string, string> &argval,
                                     string &response,
                                     ServerController &server) {
  map<string, string>::const_iterator it;
  ...
  if (!(ret = server.MoveGripper(atoi(it->second.c_str())))) {
    ...
    ptit->second.push_back(
        make_pair("positionInit",
                  pt::ptree(to_string(server.GetJoint(GRIPPER).initPosition))));
    ptit->second.push_back(
        make_pair("hightLimit",
                  pt::ptree(to_string(server.GetJoint(GRIPPER).limitHight))));
    ptit->second.push_back(make_pair(
        "lowLimit", pt::ptree(to_string(server.GetJoint(GRIPPER).limitLow))));
    ptit->second.push_back(make_pair("status", pt::ptree(string("OK"))));
  }
  _generate_json_output(&gripperroot, response);
  return true;
...
bool RestHttp::_control_wrist_rest(const string &url,
                                   const map<string, string> &argval,
                                   string &response, ServerController &server) {
  map<string, string>::const_iterator it;
  ptree::iterator ptit;
  ...
  if (it2->second == "move") {
    if (!(ret = server.MoveWrist(atoi(it->second.c_str())))) {
      ptit->second.push_back(make_pair(
          "positionCurrent",
          pt::ptree(to_string(server.GetJoint(WRIST).actualPosition))));
      ptit->second.push_back(make_pair("status", pt::ptree(string("Failed"))));
      _generate_json_output(&wristroot, response);
      return false;
    } else {
      ...
      ptit->second.push_back(
          make_pair("positionInit",
                    pt::ptree(to_string(server.GetJoint(WRIST).initPosition))));
      ptit->second.push_back(
          make_pair("hightLimit",
                    pt::ptree(to_string(server.GetJoint(WRIST).limitHight))));
      ptit->second.push_back(make_pair(
          "lowLimit", pt::ptree(to_string(server.GetJoint(WRIST).limitLow))));
      ptit->second.push_back(make_pair("status", pt::ptree(string("OK"))));
    }
    ...
    if (!(ret = server.RotateWrist(atoi(it->second.c_str())))) {
      ...
      ptit->second.push_back(make_pair(
          "positionInit",
          pt::ptree(to_string(server.GetJoint(WRIST_ROT).initPosition))));
      ptit->second.push_back(make_pair(
          "hightLimit",
          pt::ptree(to_string(server.GetJoint(WRIST_ROT).limitHight))));
      ptit->second.push_back(
          make_pair("lowLimit",
                    pt::ptree(to_string(server.GetJoint(WRIST_ROT).limitLow))));
      ptit->second.push_back(make_pair("status", pt::ptree(string("OK"))));
    }
  }
  _generate_json_output(&wristroot, response);
...
void RestHttp::_generate_json_output(void *data, string &output) {
  ...
  ptree *pt = reinterpret_cast<ptree *>(data);
  write_json(ostr, *pt);
...
bool RestHttp::_validate(const void *data) {
  ...
  map<string, set<string>>::const_iterator it =
      _apiparams.find(vdata->endpoint);
